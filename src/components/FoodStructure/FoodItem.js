...
const FoodItem = ({ image, title, description, price, type, id }) => {
    const navigate = useNavigate()
    const handleOrder = (title) =>{
      navigate(`/order/${title}`,{state:""})
    }
  return (
    <div className="bg-white border border-gray-100 transition transform duration-700 hover:shadow-xl hover:scale-105 p-4 rounded-lg relative">
      <span className="bg-primary-100 border border-bg-primary-500 rounded-full text-primary text-sm poppins px-4 py-1 inline-block mb-4 ">
        {type}
      </span>
      <img
